...
export const typeofExps = (exps: Exp[], tenv: TEnv): TExp | Error =>
    ...
    isDefineExp(first(exps)) ? typeofExps(rest(exps),makeExtendTEnv([first(exps).var.var],[first(exps).var.texp], tenv)) :
...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): TExp | Error => {
    ...
    const exTenv=makeExtendTEnv([exp.var.var],[exp.var.texp], tenv);
    const constraint1 = checkEqualType(typeofExps([exp.val],exTenv), varTE, exp);
    if (isError(constraint1))
        return constraint1;
    else
        return makeVoidTExp();
