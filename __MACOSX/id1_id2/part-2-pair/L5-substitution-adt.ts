...
import { eqTVar, isAtomicTExp, isProcTExp, isTVar,isPairTExp, makePairTExp, makeProcTExp, unparseTExp, TExp, TVar } from "./TExp";
...
export const checkNoOccurrence = (tvar: TVar, te: TExp): true | Error => {
    const check = (e: TExp): true | Error =>
        ...
        isAtomicTExp(e) ? true :
        isPairTExp(e) ? (hasNoError(map(check,[e.car,e.cdr]))?true:
                        Error(getErrorMessages(map(check,[e.car,e.cdr]))))://////////////////
...
export const applySub = (sub: Sub, te: TExp): TExp =>
    isEmptySub(sub) ? te :
    isAtomicTExp(te) ? te :
    isPairTExp(te)? makePairTExp(applySub(sub,te.car),
    applySub(sub, te.cdr)):
