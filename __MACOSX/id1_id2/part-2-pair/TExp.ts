...
import { isArray, isBoolean, isEmpty, isString, LitExp } from './L5-ast';
import { makeBox, setBox, unbox, Box } from './box';
import { isCompoundSExp , SExp} from './L5-value';
...
export type PairTExp = { tag: "PairTExp" , car: TExp , cdr: TExp }
export const isPairTExp = (x: any): x is PairTExp => x.tag === "PairTExp";
export const makePairTExp = (car: TExp, cdr: TExp): PairTExp =>
    ({tag: "PairTExp", car: car, cdr: cdr});
export const pairTExpComponents = (pair :PairTExp): TExp[] =>
    [pair.car,pair.cdr];
...
;; parseTExp(Pair T1  T2)=>'(pair-te ((tvar T1)(tvar T2))
...
*/
const parseCompoundTExp = (texps: any[]): ProcTExp | Error | PairTExp => {
    if(texps.length==3&& texps[0]==="Pair"){
        let car = parseTExp(texps[1]);
        let cdr = parseTExp(texps[2]);
        return isError(car) ? car :
        isError(cdr) ? cdr :
        makePairTExp(car,cdr); 
    }
...
export const typeOfLit = (lit: LitExp): TExp | Error => {
    return typeofSExps(lit.val);
}

export const typeofSExps = (val: SExp): TExp | Error => {
    return (typeof val) === "number" ? makeNumTExp() :
    (typeof val) === "string" ? makeStrTExp() :
    (typeof val) === "boolean" ? makeBoolTExp() :
    isCompoundSExp(val) && val.val.length===3 ? ((left:TExp|Error ,right:TExp | Error): PairTExp | Error =>
        isError(left) || isError(right) ? Error() :
        makePairTExp(left,right))(typeofSExps(val.val[0]), typeofSExps(val.val[2])) : Error();
}
...
export const unparseTExp = (te: TExp | Error): string | Error => {
    ...
    const up = (x: TExp | Error): string | string[] | Error =>
		...
		isPairTExp(x) ? [ 'Pair' , unparseTExp(x.car) , unparseTExp(x.cdr) ]:
...
export const matchTVarsInTE = <T1, T2>(te1: TExp, te2: TExp,
    ...
    (isPairTExp(te1) || isPairTExp(te2)) ? matchTVarsInTPair( te1, te2, succ, fail ) :
...
const matchTVarsInTPair = <T1, T2>(te1: TExp, te2: TExp,
        succ: (mapping: Array<Pair<TVar, TVar>>) => T1,
        fail: () => T2): T1 | T2 =>
        (isPairTExp(te1) && isPairTExp(te2))?
		matchTVarsInTEs( [te1.car,te1.cdr],[te2.car,te2.cdr], succ, fail ):fail();///////////////////////////
